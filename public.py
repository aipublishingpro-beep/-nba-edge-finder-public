...
# APP
# ============================================================
st.title("🎯 NBA EXTREME TOTALS - EDGE FINDER")
...
with st.sidebar:
    ...
    if advanced:
        if not CRYPTO_AVAILABLE:
            st.error("⚠️ Trading requires cryptography library")
        else:
...
if error:
    ...
    # 🟢 GREEN WATCHLIST
    if watchlist_green:
        st.markdown("## 🟢 WATCHLIST - PRICE OK (≤68¢)")
        ...
        for m in watchlist_green:
            ...
            if spiked or is_spiked(m["ticker"]):
                st.error(f"🛑 **PRICE SPIKE DETECTED — SKIP THIS MARKET**\n\n**🏀 {m['away_team']} @ {m['home_team']}**\n\nPrice jumped **+{int(delta)}¢** in 30 seconds!")
                if st.button(f"✅ Clear spike - {m['ticker']}", key=f"clear_{m['ticker']}"):
                    clear_spike(m["ticker"])
                    st.rerun()
            else:
                st.success(f"**🏀 {m['away_team']} @ {m['home_team']}**\n\n⭐ WATCHLIST: **{wl_team}**\n\n**Threshold:** {m['threshold']} | **NO Price:** {int(m['no_ask'])}¢ | **Live:** {live_str}")
    ...
    # 🟡 YELLOW WATCHLIST
    if watchlist_yellow:
        st.markdown("## 🟡 WATCHLIST - PRICE HIGH (>68¢)")
        st.markdown("##### ⭐ Structural brakes but needs Q1 confirmation")
        for m in watchlist_yellow:
            ...
            spiked, delta = check_price_spike(m["ticker"], m["no_ask"])
            if spiked or is_spiked(m["ticker"]):
                st.error(f"🛑 **PRICE SPIKE — SKIP**\n\n**🏀 {m['away_team']} @ {m['home_team']}**")
                if st.button(f"✅ Clear spike - {m['ticker']}", key=f"clear_y_{m['ticker']}"):
                    clear_spike(m["ticker"])
                    st.rerun()
            else:
                st.warning(f"**🏀 {m['away_team']} @ {m['home_team']}**\n\n⭐ WATCHLIST: **{wl_team}**\n\n**Threshold:** {m['threshold']} | **NO Price:** {int(m['no_ask'])}¢ | **Live:** {live_str}")
                if m["no_ask"] <= 70:
